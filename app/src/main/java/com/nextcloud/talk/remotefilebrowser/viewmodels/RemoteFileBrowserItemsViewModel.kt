...
class RemoteFileBrowserItemsViewModel @Inject constructor(
    ...
    private lateinit var repositoryParameters: RemoteFileBrowserItemsRepository.Parameters
    ...
    fun loadItems(path: String) {
        repositoryParameters = RemoteFileBrowserItemsRepository.Parameters(path)
        loadItems(repositoryParameters)
    }

    private fun loadItems(repositoryParameters: RemoteFileBrowserItemsRepository.Parameters) {
        _viewState.value = LoadingItemsState
        repository.listFolder(repositoryParameters)?.subscribeOn(Schedulers.io())
